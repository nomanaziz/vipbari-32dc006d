      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          <span className="inline-block bg-primary/10 text-primary text-sm font-medium px-4 py-1.5 rounded-full mb-4">
            {lc("benefit_badge", "landing.benefit_badge")}
          </span>
          <h2 className="text-3xl font-bold mb-3">{lc("benefit_title", "landing.benefit_title")}</h2>
        </div>
          ...
          {items.map((item) => (
            <Card key={item.key} className="hover:shadow-md transition-shadow">
              <CardContent className="p-6 flex flex-col items-center text-center">
                ...
                <h3 className="font-semibold mt-4 mb-1">{lc(`benefit_${item.key}`, `landing.benefit_${item.key}`)}</h3>
                <p className="text-sm text-muted-foreground">{lc(`benefit_${item.key}_desc`, `landing.benefit_${item.key}_desc`)}</p>
              </CardContent>
            </Card>
          ))}
        </div>
