...
import { DoorOpen, Send, CalendarClock, MapPin, Flame, Car, Bike, ExternalLink, Building2, BedDouble, Bath, Maximize, Home, ArrowUpDown, Wifi, Zap } from "lucide-react";
...
const ToLet = () => {
  ...
  const { data: rooms, isLoading: roomsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("rooms")
        .select("*, properties!inner(name, address, owner_id, division, district, thana, area, tolet_phone, map_url, has_garage, has_internet, has_dish, has_security, has_cctv, has_lift, has_generator, has_parking, has_gas_supply, has_water_supply, has_rooftop_access), room_images(image_url, sort_order)")
        .eq("is_tolet", true);
  ...
  const { data: garages, isLoading: garagesLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .eq("status", "vacant");
      if (error) throw error;
      return (data || []).map((g: any) => ({ ...g, _type: "garage" as const, rent_amount: g.rent_amount }));
    },
  });
  ...
  // Room-specific filters (only apply to rooms)
  ...
  // Garage-specific filter
  if (filters.garageType) filtered = filtered.filter((r: any) => r._type === "room" || r.garage_type === filters.garageType);
  if (availability === "now") filtered = filtered.filter((r: any) => r._type === "garage" || r.status === "vacant");
  else if (availability === "upcoming") filtered = filtered.filter((r: any) => r._type === "room" && r.status === "occupied" && r.available_from);
  ...
  // Random boost picks for current page
  ...
  const handleRequestClick = (room: any) => {
    if (!user) { navigate("/register?role=tenant"); return; }
    if (role !== "tenant") { toast.error(t("tolet.tenant_only") || "Only tenants can send rental requests"); return; }
    setRequestDialog(room);
  };
  ...
  const renderListingCard = (item: any, isBoosted: boolean) => {
    ...
    const availBadge = isGarage ? (
      ...
      <Badge className="absolute bottom-2 left-2 z-10 bg-orange-500 text-white text-[10px] px-1.5 py-0.5 shadow gap-0.5">
        <CalendarClock className="h-2.5 w-2.5" />
        {format(new Date(room.available_from), "dd MMM")}
      </Badge>
    ) : null;
      ...
      <>
        ...
        <div className="p-3 flex flex-col flex-1 gap-2">
          ...
          <div className="mt-auto pt-1">
            <Button size="sm" className="h-9 text-sm gap-1.5 w-full" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleRequestClick(item); }}>
    ...
    const WrapperCard = isBoosted ? (
      <GlowCard key={`${item._type}-${item.id}`} glowColor="purple" customSize className="w-full h-full aspect-auto">
        <Link to={`/tolet/${item.id}`} className="block h-full">
          <Card className="hover:shadow-md transition-shadow overflow-hidden border-0 shadow-none bg-transparent flex flex-col h-full rounded-2xl">
            {cardContent}
          </Card>
        </Link>
      </GlowCard>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold mb-1">{t("tolet.title") || "To-Let Listings"}</h1>
          <p className="text-muted-foreground text-sm">{t("tolet.subtitle") || "Find your next home — browse vacant rooms and flats"}</p>
        </div>
                ...
                availabilitySlot={
                  <Select value={availability} onValueChange={setAvailability}>
                    <SelectTrigger className="h-9 text-xs w-full">
                      <SelectValue placeholder={language === "bn" ? "প্রাপ্যতা" : "Availability"} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">{language === "bn" ? "সব" : "All"}</SelectItem>
                      <SelectItem value="now">{language === "bn" ? "এখন খালি" : "Available Now"}</SelectItem>
                      <SelectItem value="upcoming">{language === "bn" ? "আসছে" : "Upcoming"}</SelectItem>
                    </SelectContent>
                  </Select>
                }
              />
            ...
            {isLoading ? (
              ...
              <Card>
                <CardContent className="p-12 text-center text-muted-foreground">
                  ...
                  <p>{t("tolet.no_listings") || "No to-let listings found. Try adjusting your filters."}</p>
                </CardContent>
              </Card>
            ) : (
          ...
          {requestDialog && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder={t("tolet.message_placeholder") || "Introduce yourself and mention your requirements..."} rows={3} />
              </div>
              <div className="flex gap-2 justify-end">
                <Button variant="outline" onClick={() => setRequestDialog(null)}>{t("common.cancel")}</Button>
                <Button
                  onClick={() => {
                    const landlordId = (requestDialog as any).properties?.owner_id;
                    if (!landlordId) {
                      supabase.from("properties").select("owner_id").eq("id", requestDialog.property_id).single().then(({ data }) => {
                        if (data) requestMutation.mutate({ roomId: requestDialog.id, landlordId: data.owner_id });
                      });
                    } else {
                      requestMutation.mutate({ roomId: requestDialog.id, landlordId });
                    }
                  }}
      ...
      {(!user || role === "tenant") && filtered.length > 0 && filtered[0]?.properties?.owner_id && (
        <VisitorChatWidget landlordId={filtered[0].properties.owner_id} roomId={filtered[0]?.id} />
      )}
